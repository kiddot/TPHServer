...
public final class SingleUserPushTask implements PushTask, ChannelFutureListener {
    ...
    public SingleUserPushTask(IPushMessage message, Connection connection) {
    ...
    private boolean checkLocal(IPushMessage message) {
        ...
        if (true) {
            ...
            //5.链接可用，直接下发消息到手机客户端
            PushMessage pushMessage = PushMessage.build(mConnection).setContent(message.getContent());
            System.out.println("SIngleUserPUshTAsk:" + mConnection + pushMessage);
...
//        }
    ...
    public void operationComplete(ChannelFuture future) throws Exception {
        ...
        if (future.isSuccess()) {//推送成功
            ...
            //("[SingleUserPush] push message to client failure, message={}, conn={}", message, future.channel());
        }
    }
